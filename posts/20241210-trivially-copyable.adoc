...
This article explores how we can emulate a C++-like concept of _Trivially Copyable Objects_ within Java to achieve far more efficient serialisation.
...
C++ defines a _Trivially Copyable Object_ as one that can be safely cloned by copying its memory representation directly (e.g. using `memcpy`).
...
The result is similar to how C++ handles trivially copyable types, but now we are in Java's safe, garbage-collected environment.
...
Bulk copying here is the key to near C++-style efficiency.
...
Java may not natively support trivially copyable objects, but we can still achieve near C++-like serialisation speeds by restructuring data and using Chronicle's low-level operations.
